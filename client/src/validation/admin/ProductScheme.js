...
export const addProductScheme = Yup.object().shape({
    productName: Yup.string().required("Product Name Is Required").min(2, "Product Name Atleast Have More Than 2 Characters"),
    productSlug: Yup.string().required("Product Slug Is Required"),
    basePrice: Yup.string()
        .test("is-valid-price", "Product Price Is Required And Must Be A Positive Number", function (value) {
            const { productType } = this.parent;
            // Only validate productStock if productType is "simple"
            if (productType === "simple" && (!value || Number(value) <= 0 || isNaN(Number(value)))) {
    ...
    productDiscountPrice: Yup.string()
        .test("is-valid-discount-price", "Product Discount Price Is Required And Must Be A Positive Number", function (value) {
            const { productType } = this.parent;
            // Only validate productStock if productType is "simple"
            if (productType === "simple") {
                ...
                if (isNaN(numericValue) || numericValue <= 0) {
                    return this.createError({
                        message: "Product Discount Price Must Be A Positive Number.",
                    });
                }
            }
            return true;
    ...
    productStock: Yup.string()
        .test("is-valid-stock", "Product Stock Is Required And Must Be A Positive Number", function (value) {
            const { productType } = this.parent;
            // Only validate productStock if productType is "simple"
            if (productType === "simple") {
                if (!value) {
        ...
        .notRequired(),
    productCategoryId: Yup.string().required("Product Category Is Required").notOneOf(["", "default"], "You Must Select A Valid Category"),
    productSubCategoryId: Yup.string().required("Product Sub Category Is Required").notOneOf(["", "default"], "You Must Select A Valid Sub-Category"),
    ...
    productSpecification: Yup.string().required("Product Specification Is Required").min(5, "Product Specification Atleast Have More Than 5 Characters"),
});
