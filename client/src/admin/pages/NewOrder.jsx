...
const NewOrder = () => {
    ...
    const orderData = data?.data?.map((order, index) => ({ no: index + 1, ...order })) || [];

    if (isPending) return <Loader />;
    return (
        <>
            <PageHeader title={"Manage Order's"} controller={"Order"} controllerUrl={"/admin/orders/new-order/"} page={"New Order's"} />
